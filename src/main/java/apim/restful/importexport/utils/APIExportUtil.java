...
public final class APIExportUtil {
     ...
     *
     * @param apiID
     *            Identifier of the exporting API
     * @param userName
	...
	try {
	    ...
	    return Response.status(Response.Status.NOT_FOUND).entity("Unable to retrieve API").type(MediaType.APPLICATION_JSON).build();
	}
	...
	// export sequences
	exportSequences(apiToReturn, apiID, APIUtil.getTenantId(userName));
     ...
     *
     * @param apiIdentifier
     *            ID of the requesting API
     * @param registry
     *            Current tenant registry
     * @throws APIExportException
     *             If an error occurs while retrieving image from the registry or storing in the archive directory
     */
	...
	try {
	    ...
	    // Exception is ignored by logging due to the reason that Thumbnail is not essential for an API to be recreated
     ...
     *
     * @param mediaType
     *            Media type of the thumbnail registry resource
    ...
    /**
     * Retrieve documentation for the exporting API and store it in the archive
     * directory FILE, INLINE and URL documentations are handled
     *
     * @param apiIdentifier
     *            ID of the requesting API
     * @param registry
     *            Current tenant registry
     * @param docList
     *            documentation list of the exporting API
     * @throws APIExportException
     *             If an error occurs while retrieving documents from the
     *             registry or storing in the archive directory
     */
    public static void exportAPIDocumentation(List<Documentation> docList, APIIdentifier apiIdentifier, Registry registry) throws APIExportException {
	String archivePath = archiveBasePath.concat(File.separator + apiIdentifier.getApiName() + "-" + apiIdentifier.getVersion());
	...
	try {
		...
		if (Documentation.DocumentSourceType.FILE.toString().equalsIgnoreCase(sourceType)) {
		    String fileName = doc.getFilePath().substring(doc.getFilePath().lastIndexOf(RegistryConstants.PATH_SEPARATOR) + 1);
     ...
     *            Current tenant registry
     * @throws APIExportException
     *             If an error occurs while retrieving WSDL from the registry or
	...
	OutputStream outputStream = null;
	try {
	    String wsdlPath = APIConstants.API_WSDL_RESOURCE_LOCATION + apiIdentifier.getProviderName() + "--" + apiIdentifier.getApiName() + apiIdentifier.getVersion() + ".wsdl";
	    if (registry.resourceExists(wsdlPath)) {
		String apiNameVersion = apiIdentifier.getApiName() + "-" + apiIdentifier.getVersion();
		String archivePath = archiveBasePath.concat(File.separator + apiNameVersion);
		String wsdlDirectoryPath = archivePath + File.separator + "WSDL";
		createDirectory(wsdlDirectoryPath);

		Resource wsdl = registry.get(wsdlPath);
		wsdlStream = wsdl.getContentStream();

		outputStream = new FileOutputStream(wsdlDirectoryPath + File.separator + apiNameVersion + ".wsdl");
     ...
     *            ID of the requesting API
     * @throws APIExportException
     *             If an error occurs while retrieving sequences from registry
     */
	...
	if (!sequences.isEmpty()) {
		...
		for (Map.Entry<String, String> sequence : sequences.entrySet()) {
			...
			if (APIConstants.API_CUSTOM_SEQUENCE_TYPE_IN.equalsIgnoreCase(direction)) {
			    sequenceConfig = APIUtil.getCustomSequence(sequenceName, tenantId, APIConstants.API_CUSTOM_SEQUENCE_TYPE_IN);
			    writeSequenceToFile(sequenceConfig, sequenceName, APIConstants.API_CUSTOM_SEQUENCE_TYPE_IN, apiIdentifier);
			} else if (APIConstants.API_CUSTOM_SEQUENCE_TYPE_OUT.equalsIgnoreCase(direction)) {
			    sequenceConfig = APIUtil.getCustomSequence(sequenceName, tenantId, APIConstants.API_CUSTOM_SEQUENCE_TYPE_OUT);
			    writeSequenceToFile(sequenceConfig, sequenceName, APIConstants.API_CUSTOM_SEQUENCE_TYPE_OUT, apiIdentifier);
			} else {
			    sequenceConfig = APIUtil.getCustomSequence(sequenceName, tenantId, APIConstants.API_CUSTOM_SEQUENCE_TYPE_FAULT);
			    writeSequenceToFile(sequenceConfig, sequenceName, APIConstants.API_CUSTOM_SEQUENCE_TYPE_FAULT, apiIdentifier);
			}
		    }
		}
	    } catch (APIManagementException e) {
     ...
     *            Sequence name
     * @param direction
     *            Direction of the sequence "in", "out" or "fault"
     * @param apiIdentifier
     *            ID of the requesting API
     * @throws APIExportException
     *             If an error occurs while serializing XML stream or storing in
     *             archive directory
     */
    public static void writeSequenceToFile(OMElement sequenceConfig, String sequenceName, String direction, APIIdentifier apiIdentifier) throws APIExportException {
	OutputStream outputStream = null;
    ...
    /**
     * Retrieve meta information of the API to export URL template information
     * are stored in swagger.json definition while rest of the required data are
     * in api.json
     *
     * @param apiToReturn
     *            API to be exported
     ...
     *            Current tenant registry
     * @throws APIExportException
     *             If an error occurs while exporting meta information
     */
	...
	String metaInformationPath = archivePath + File.separator + "Meta-information";
	createDirectory(metaInformationPath);

	Gson gson = new GsonBuilder().setPrettyPrinting().create();
	String apiInJson = gson.toJson(apiToReturn);
	writeFile(metaInformationPath + File.separator + "api.json", apiInJson);
	...
	try {
	    ...
	    writeFile(metaInformationPath + File.separator + "swagger.json", formattedSwaggerJson);
