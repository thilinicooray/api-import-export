 ...
 */
public final class ArchiveGeneratorUtil {
     ...
     *             If an error occurs while generating archive
     */
     ...
     * @throws APIExportException
     *             If an error occurs while adding files to the archive
     */
     ...
     *            Location of the archive
     * @param file
     *            File to be included in the archive
     * @param zipOutputStream
     *            Output stream
     * @throws APIExportException
     *             If an error occurs while writing files to the archive
     */
	...
	try {
	    ...
	    ZipEntry zipEntry = new ZipEntry(getZipEntryName(zipFilePath));
    ...
    /**
     * Replace Windows file separator character with '/'
     * 
     * @param zipFilePath
     *            Relative file path
     * @return Zip entry name
     */
    private static String getZipEntryName(String zipFilePath) {
	if (zipFilePath != null && File.separatorChar != APIImportExportConstants.ZIP_ENTRY_SEPARATOR) {
	    return zipFilePath.replace('\\', APIImportExportConstants.ZIP_ENTRY_SEPARATOR);
	} else {
	    return zipFilePath;
	}
    }
