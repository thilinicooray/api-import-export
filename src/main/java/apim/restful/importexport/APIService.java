...
public class APIService {
    ...
    public Response exportAPI(@QueryParam("name") String name,
	...
	try {
	    ...
	    String createdFolders = File.separator + RandomStringUtils.randomAlphanumeric(APIImportExportConstants.TEMP_FILENAME_LENGTH) + File.separator;
	    ...
	    // Retrieve resources : thumbnail, meta information, wsdl, sequences and documents available for the exporting API
     ...
     * @param preserveProviderStatus
     *            user choice to keep or replace the API provider
     * @param preserveGatewayStatus
     *            user choice to keep or replace the API Gateway Environments
     * @param httpHeaders
    ...
    public Response importAPI(@Multipart("file") InputStream uploadedInputStream,
	    		      @QueryParam("preserveProvider") String preserveProviderStatus,
	    		      @QueryParam("preserveGateway") String preserveGatewayStatus,
	    		      @Context HttpHeaders httpHeaders) {

	boolean isProviderPreserved = true;
	boolean isGatewayPreserved = true;
	// Check if the 'preserveProvider' URL parameter value is specified, otherwise the default value "true" is used
	if (APIImportExportConstants.STATUS_FALSE.equalsIgnoreCase(preserveProviderStatus)) {
	    isProviderPreserved = false;
	}
	// Check if the 'preserveGateway' URL parameter value is specified, otherwise the default value "true" is used
	if (APIImportExportConstants.STATUS_FALSE.equalsIgnoreCase(preserveGatewayStatus)) {
	    isGatewayPreserved = false;
	}
	...
	try {
		...
		if (folderCreateStatus) {
		    ...
		    APIImportUtil.importAPI(absolutePath + extractedFolderName, currentUser, isProviderPreserved, isGatewayPreserved);
